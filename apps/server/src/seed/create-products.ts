		...
		try {
			...
			if (lines.length > 0) {
				...
				for (const [_i, line] of lines.entries()) {
					...
					const defaultRate = Number.parseFloat(cells[idx.defaultRate] || "0");
					const hsnCode = cells[idx.hsnCode] || "";
					const gstPercentage = Number.parseFloat(
						cells[idx.gstPercentage] || "0",
					);
					if (!name) continue;
			...
			console.warn(
				"Failed to read products.csv, falling back to sample list",
				err,
			);
		}
	}
