		...
		orpc.uploadBuyersCSV?.mutationOptions?.({
			onSuccess: (res: any) => {
				...
				if (res?.existingNames?.length) {
					...
					toast.error(`Buyers already exist: ${names}`, {
						position: "top-left",
					});
				}
				if (res?.insertedCount) {
					...
					toast.success(`Inserted ${res.insertedCount} buyers`, {
						description,
						position: "top-right",
					});
				} else {
		...
		<div className="flex h-full flex-col">
			<div className="mb-6 flex items-center justify-between">
				<div className="flex items-center gap-3">
					...
					<div>
						<h1 className="font-bold text-3xl tracking-tight">Buyers</h1>
						...
						<div className="flex-1 overflow-y-auto">
							<div className="space-y-6 pb-6">
								<div className="flex items-center justify-between">
									<h2 className="font-bold text-2xl">Buyer Details</h2>
									<Button
										variant="outline"
								...
								<div className="grid gap-6">
									<div className="grid grid-cols-1 gap-6 md:grid-cols-2">
										...
										<div className="space-y-2">
											...
											<p className="text-muted-foreground text-sm">
												{new Date(selectedBuyer.createdAt).toLocaleString(
													"en-IN",
												)}
											</p>
										</div>
										...
										{selectedBuyer.updatedAt && (
											<div className="space-y-2">
												...
												<p className="text-muted-foreground text-sm">
													{new Date(selectedBuyer.updatedAt).toLocaleString(
														"en-IN",
													)}
												</p>
											</div>
				...
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							...
							<strong>{buyerToDelete?.name}</strong>? This action cannot be
							undone.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
