...
async function seedBuyers() {
	...
	const buyersToInsert: Array<any> = [];
	...
	if (fs.existsSync(csvPath)) {
		try {
			...
			if (lines.length > 0) {
				...
				const header = parseCsvLine(lines.shift()!).map((h) =>
					h.trim().toLowerCase(),
				);
				const idx = {
				...
				for (const [_i, line] of lines.entries()) {
					...
					const totalInvoices = totalInvoicesRaw
						? Number.parseInt(totalInvoicesRaw, 10) || 0
						: 0;
			...
			console.warn(
				"Failed to read buyers.csv, falling back to synthetic seed",
				err,
			);
		}
	}
	...
	if (buyersToInsert.length > 0) {
		...
		console.log(
			`✅ Successfully seeded ${buyersToInsert.length} buyers from CSV`,
		);
	...
	const sampleCompanyNames = [
		...
		"Wellness Corp",
		"Prime Medical Enterprises",
		"Advanced Healthcare Systems",
		"Unity Pharmaceuticals",
		"Reliable Medical Co",
		"Modern Healthcare Distributors",
