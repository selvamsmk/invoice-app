    ...
    orpc.uploadProductsCSV?.mutationOptions?.({
      onSuccess: (res: any) => {
        ...
        if (res?.existingNames?.length) {
          ...
          toast.error(`Products already exist: ${names}`, { position: 'top-left' })
        }
        if (res?.insertedCount) {
          const names = res.insertedNames?.join(', ') || ''
          const description = names.length > 0 ? names : undefined
          toast.success(`Inserted ${res.insertedCount} products`, { description, position: 'top-right' })
        } else {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Manage your product catalog and pricing information</p>
          </div>
        </div>
                ...
                <TableBody>
                  {products.data?.map((product) => (
                    <TableRow key={product.id}>
                      <TableCell className="font-medium w-[250px]">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div className="truncate cursor-help">{product.name}</div>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>{product.name}</p>
                          </TooltipContent>
                      ...
                      <TableCell className="w-[120px]">₹{product.defaultRate.toFixed(2)}</TableCell>
                      <TableCell className="w-[120px]">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div className="truncate cursor-help">{product.hsnCode}</div>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>{product.hsnCode}</p>
                          </TooltipContent>
                      ...
                      <TableCell className="w-[100px]">{product.gstPercentage}%</TableCell>
              ...
              <div className="space-y-6 pb-6">
                ...
                <div className="grid gap-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-sm font-medium text-muted-foreground">Default Rate</label>
                      <p className="text-lg font-semibold">₹{selectedProduct.defaultRate.toFixed(2)}</p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-lg">{selectedProduct.gstPercentage}%</p>
                    </div>
                    ...
                    {selectedProduct.updatedAt && (
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {new Date(selectedProduct.updatedAt).toLocaleString('en-IN')}
                        </p>
                      </div>
                    )}
                  </div>
