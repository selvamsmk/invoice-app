...
import { invoiceLineItem, invoiceLineItemBatch, product, buyer as buyerModel } from "@invoice-app/db";
...
export function mapInvoiceDataToInvoiceProps(invoiceData: DbInvoice): InvoiceProps {
  const lineItemsForPdf = (invoiceData.lineItems || []).map((li: any) => {
    const totalQuantity = (li.batches || []).reduce((s: number, b: any) => s + (b.quantity || 0), 0);
    return {
      id: li.id,
      name: li.productName ?? li.product?.name ?? '',
      hsnCode: li.hsnCode ?? li.product?.hsnCode ?? '',
      expiryDate: (li.batches && li.batches.length > 0) ? (li.batches.map((b: any) => b.expiryDate).filter(Boolean).join(', ')) : undefined,
      quantity: totalQuantity,
      batches: (li.batches || []).map((b: any) => ({ id: b.id, batchNo: b.batchNo ?? null, expiryDate: b.expiryDate ?? null, quantity: b.quantity ?? 0 })),
  ...
  const selectedInvoiceForPdf: InvoiceProps = {
    ...
    createdAt: invoiceData.createdAt ? (invoiceData.createdAt instanceof Date ? invoiceData.createdAt.toISOString() : String(invoiceData.createdAt)) : new Date().toISOString(),
