...
import { db, product, eq } from "@invoice-app/db";
import { z } from "zod";
...
export const productsRouter = {
  ...
  deleteProduct: publicProcedure
    ...
    .handler(async ({ input }) => {
      const [deletedProduct] = await db.delete(product).where(eq(product.id, input.id)).returning();
  ...
  uploadProductsCSV: publicProcedure
    ...
    .handler(async ({ input }) => {
      ...
      const rows: Array<{ name: string; defaultRate: number; hsnCode: string; gstPercentage: number }> = [];
      ...
      for (const [i, line] of lines.entries()) {
        ...
        const defaultRate = parseFloat(defaultRateRaw);
        if (Number.isNaN(defaultRate) || defaultRate <= 0) throw new Error(`Row ${i + 2}: defaultRate must be a positive number`);
        if (!hsnCode) throw new Error(`Row ${i + 2}: hsnCode is required`);
        const gstPercentage = parseFloat(gstRaw);
        if (Number.isNaN(gstPercentage) || gstPercentage < 0 || gstPercentage > 100) throw new Error(`Row ${i + 2}: gstPercentage must be between 0 and 100`);
