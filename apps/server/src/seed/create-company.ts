...
async function seedCompany() {
	...
	if (!fs.existsSync(csvPath)) {
		console.warn(
			"company.csv not found. Using dummy data. Copy company.csv.example to company.csv for custom details.",
		);
	} else {
		try {
			...
			if (lines.length < 2 || !lines[0] || !lines[1]) {
				...
				const data = parseCsvLine(lines[1]).map((c) =>
					c.trim().replace(/^"|"$/g, ""),
				);
