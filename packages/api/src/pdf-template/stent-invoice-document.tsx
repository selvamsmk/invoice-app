...
export interface StentInvoiceProps {
	id: string;
...
interface StentInvoiceDocumentProps {
	selectedInvoice: StentInvoiceProps;
	companyData?: Company;
}

export function StentInvoiceDocument({
	selectedInvoice,
	companyData,
}: StentInvoiceDocumentProps) {
	return (
			...
			<Page size="A4" style={pdfStyles.page} wrap>
				...
				<View style={pdfStyles.headerRow} fixed>
					...
					<View style={pdfStyles.invoiceInfo}>
						<Text style={pdfStyles.invoiceTypeText}>
				...
				<View style={pdfStyles.sectionBlock} fixed>
					<View style={pdfStyles.infoRow}>
						<View style={[pdfStyles.infoColumn, pdfStyles.billToColumn]}>
							...
							{selectedInvoice.buyerAddressLine2 && (
								<Text>{selectedInvoice.buyerAddressLine2}</Text>
							)}
							{(selectedInvoice.buyerCity ||
								selectedInvoice.buyerState ||
								selectedInvoice.buyerPincode) && (
								<Text>
									{[
										selectedInvoice.buyerCity,
										selectedInvoice.buyerState,
							...
							{selectedInvoice.buyerPhone && (
								<Text>Phone: {selectedInvoice.buyerPhone}</Text>
							)}
							{selectedInvoice.buyerGstin && (
								<Text>GSTIN: {selectedInvoice.buyerGstin}</Text>
							)}
						</View>
						...
						<View style={[pdfStyles.infoColumn, pdfStyles.invoiceMetaColumn]}>
							...
							<View style={pdfStyles.invoiceMetaGroup}>
								<View style={pdfStyles.metaRow}>
									...
									<Text style={pdfStyles.metaValue}>
										{[
											selectedInvoice.dcNumber,
											selectedInvoice.dcDate
												? formatDate(selectedInvoice.dcDate)
												: "",
										]
											.filter(Boolean)
											.join(" & ")}
				...
				{/* Products table with patient info and sizes */}
				<View style={[pdfStyles.sectionBlock, pdfStyles.itemsTableContainer]}>
					<View style={pdfStyles.itemsTable}>
							...
							<>
								{selectedInvoice.lineItems.map((item, idx) => {
									...
									const validSizes = sizes.filter(
										(s: any) => s && s.sizeDimension && s.serialNumber,
									);

									const rowStyles: any = [pdfStyles.itemsRow];
									const isLastItem =
										idx === selectedInvoice.lineItems!.length - 1;
									if (validSizes.length > 0 || isLastItem)
									...
									const patientInfo = (
										<>
											...
											<Text>
												...
												<Text style={{ fontWeight: "bold" }}>
													{item.patientAge} /{" "}
													{item.patientGender === "male" ? "Male" : "Female"}
												</Text>
											</Text>
											<Text>
												Date:{" "}
										...
										<View key={item.id ?? idx}>
											<View style={rowStyles} wrap={false}>
												...
												<View style={[pdfStyles.itemsCell, pdfStyles.colItem]}>
													<Text>{item.name}</Text>
													<View style={pdfStyles.patientSubRow}>
														{patientInfo}
													</View>
												</View>
												<Text
												...
												>
													{validSizes.map((s) => s.sizeDimension).join("\n")}
												</Text>
												<Text
													style={[
														pdfStyles.itemsCell,
												...
												>
													{validSizes.map((s) => s.serialNumber).join("\n")}
												</Text>
												<Text
													style={[
														pdfStyles.itemsCell,
												...
												>
													{validSizes
														.map((s) => formatExpiry(s.expiryDate ?? undefined))
														.filter(Boolean)
														.join("\n")}
												</Text>
												<Text
						...
						<View style={pdfStyles.itemsSpacerRow}>
							...
							<View style={[pdfStyles.itemsCell, pdfStyles.colSize]} />
							<View style={[pdfStyles.itemsCell, pdfStyles.colSerial2]} />
					...
					{(() => {
						...
						const totalTax = items.reduce(
							(s, it) =>
								...
								(typeof it.rate === "number" ? it.rate : 0) *
									(typeof it.quantity === "number" ? it.quantity : 0) *
									((it.gstPercentage ?? 0) / 100),
							0,
						);
						const cgstAmount = totalTax / 2;
						const sgstAmount = totalTax / 2;
						const baseTaxRate =
							items.length > 0 ? (items[0]?.gstPercentage ?? 0) : 0;
						...
						return (
							<View style={pdfStyles.taxTableBlock}>
								<View style={[pdfStyles.taxRow, pdfStyles.itemsHeaderRow]}>
									...
									>
										...
										<View style={[pdfStyles.taxSplitHeader]}>
											...
											<Text
												style={[pdfStyles.taxHeaderAmount, pdfStyles.textRight]}
											>
												Amount
											</Text>
										</View>
									...
									<Text style={[pdfStyles.taxCell, pdfStyles.taxColTotal]}>
										Total Tax Amount
									</Text>
								</View>
								...
								<View style={pdfStyles.taxRow}>
									...
									>
										{formatCurrencyPlain(taxableAmount)}
									</Text>
									<View
										style={[
											pdfStyles.taxCell,
											pdfStyles.taxSplitRow,
											pdfStyles.borderRight,
										]}
									>
										<Text
											style={[
												pdfStyles.taxValuePercent,
												pdfStyles.borderRight,
												pdfStyles.textRight,
											]}
										>
										...
										<Text
											style={[pdfStyles.taxValueAmount, pdfStyles.textRight]}
										>
											{formatCurrencyPlain(cgstAmount)}
										</Text>
									</View>
									<View
										style={[
									...
									>
										...
										<Text
											style={[pdfStyles.taxValueAmount, pdfStyles.textRight]}
										>
											{formatCurrencyPlain(sgstAmount)}
										</Text>
									</View>
									<Text
										style={[
											pdfStyles.taxCell,
											pdfStyles.taxColTotal,
											pdfStyles.textRight,
										]}
									>
										{formatCurrencyPlain(totalTax)}
									</Text>
								</View>
							</View>
						);
							...
							<View style={pdfStyles.amountWordsRow}>
								...
								<View style={pdfStyles.amountWordsValueCell}>
									<Text style={pdfStyles.amountWordsValueText}>
										Rupees{" "}
										{capitalizeWords(
											numberToWordsINR(selectedInvoice.totalAmount),
										)}{" "}
										Only
									</Text>
								</View>
...
export const pdfStyles = StyleSheet.create({
	...
	/* Table styles - updated for stent invoice */
	itemsTableContainer: {
		display: "flex",
		flexDirection: "column",
		paddingBottom: 0,
	...
	colRate: { width: "9%" },
	colAmount: { width: "12%" },
	patientSubRow: { fontSize: FONT_SIZE_TINY, marginTop: 2 },
	itemsRowNoBorder: {
		borderLeftWidth: 1,
		borderRightWidth: 1,
		borderTopWidth: 0,
...
export default StentInvoiceDocument;
