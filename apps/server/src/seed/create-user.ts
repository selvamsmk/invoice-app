...
async function seedInitialUser() {
  ...
  if (!fs.existsSync(csvPath)) {
    console.warn("user.csv not found. Using dummy data. Copy user.csv.example to user.csv for custom credentials.");
    // Dummy data fallback
    ...
    try {
      ...
      if (lines.length < 2 || !lines[0] || !lines[1]) {
        ...
        if (!email || !password) {
          console.warn("Email or password missing in user.csv. Using dummy data.");
          email = 'admin@example.com';
          password = 'admin123';
        }
      }
